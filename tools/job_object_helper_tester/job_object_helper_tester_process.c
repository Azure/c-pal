...
int main(int argc, char* argv[])
{
    ...
    if (argc != 4) {
        (void)printf("Usage: target.exe <JobObjectName> <MaxCpuPercentage> <MaxPhysicalMemoryPercentage>\n");
        return 1;
    }
    ...
    (void)printf("Received JobName: %s\n", job_name);
    (void)printf("Received MaxCpuPercentage: %d\n", max_cpu_percentage);
    (void)printf("Received MaxPhysicalMemoryPercentage: %d\n", max_physical_memory_percentage);
