...
static int epoll_worker_func(void* parameter)
{
    ...
    // Codes_COMPLETION_PORT_LINUX_11_031: [ If parameter is NULL, epoll_worker_func shall do nothing. ]
    ...
    else
    {
        ...
        do
        {
            ...
            // Codes_COMPLETION_PORT_LINUX_11_032: [ epoll_worker_func shall call epoll_wait to wait for an epoll event to become signaled with a timeout of 2 Seconds. ]
            ...
            // Codes_COMPLETION_PORT_LINUX_11_034: [ epoll_worker_func shall loop through the num of descriptors that was returned. ]
            for (int index = 0; index < num_ready; index++)
            {
                ...
                #endif
                // Codes_COMPLETION_PORT_LINUX_11_035: [ epoll_worker_func shall call the event_callback with the specified COMPLETION_PORT_EPOLL_ACTION that was returned. ]
                epoll_data->event_callback(epoll_data->event_callback_ctx, epoll_action);
                // Codes_COMPLETION_PORT_LINUX_11_036: [ Then epoll_worker_func shall remove the EPOLL_THREAD_DATA from the list and free the object. ]
                if (remove_thread_data_from_list(completion_port, epoll_data) != 0)
                {
                    LogWarning("failure removing receive data to list %p, action: %" PRI_MU_ENUM "", epoll_data, MU_ENUM_VALUE(COMPLETION_PORT_EPOLL_ACTION, epoll_action));
                }
                free(epoll_data);
            }
            // Codes_COMPLETION_PORT_LINUX_11_033: [ On a epoll_wait timeout epoll_worker_func shall ensure it should not exit and issue another epoll_wait. ]
...
COMPLETION_PORT_HANDLE completion_port_create(void)
{
    ...
    // Codes_COMPLETION_PORT_LINUX_11_001: [ completion_port_create shall allocate memory for a completion port object. ]
    ...
    else
    {
        ...
        {
            // Codes_COMPLETION_PORT_LINUX_11_002: [ completion_port_create shall create the epoll instance by calling epoll_create. ]
            ...
            {
                // Codes_COMPLETION_PORT_LINUX_11_003: [ completion_port_create shall create a thread that runs epoll_worker_func to handle the epoll events. ]
                ...
                {
                    // Codes_COMPLETION_PORT_LINUX_11_004: [ On success completion_port_create shall return the allocated COMPLETION_PORT_HANDLE. ]
                    goto all_ok;
                }
            }
            (void)close(result->epoll);
        }
        // Codes_COMPLETION_PORT_LINUX_11_005: [ If there are any errors then completion_port_create shall fail and return NULL. ]
...
{
    // Codes_COMPLETION_PORT_LINUX_11_006: [ If completion_port is NULL, completion_port_inc_ref shall return. ]
    if (completion_port == NULL)
    {
    ...
    {
        // Codes_COMPLETION_PORT_LINUX_11_007: [ Otherwise completion_port_inc_ref shall increment the internally maintained reference count. ]
...
{
    // Codes_COMPLETION_PORT_LINUX_11_008: [ If completion_port is NULL, completion_port_dec_ref shall return. ]
    if (completion_port == NULL)
    {
        LogError("Invalid arguments: COMPLETION_PORT_HANDLE completion_port=%p", completion_port);
    }
    ...
    {
        // Codes_COMPLETION_PORT_LINUX_11_009: [ completion_port_dec_ref shall decrement the reference count for completion_port. ]
        if (DEC_REF(COMPLETION_PORT, completion_port) == 0)
        {
            // Codes_COMPLETION_PORT_LINUX_11_010: [ If the reference count reaches 0, completion_port_dec_ref shall do the following: ]
            int32_t value;
            // Codes_COMPLETION_PORT_LINUX_11_011: [ - wait for the ongoing call count to reach zero. ]
            ...
            // Codes_COMPLETION_PORT_LINUX_11_012: [ - increment the flag signaling that the threads can complete. ]
            interlocked_increment(&completion_port->worker_thread_continue);

            // Codes_COMPLETION_PORT_LINUX_11_013: [ - close the epoll object. ]
            ...
            int dont_care;
            // Codes_COMPLETION_PORT_LINUX_11_014: [ - close the thread by calling ThreadAPI_Join. ]
            if (ThreadAPI_Join(completion_port->thread_handle, &dont_care) != THREADAPI_OK)
            {
                LogError("Failure joining thread");
            }

            // Codes_COMPLETION_PORT_LINUX_11_015: [ - then the memory associated with completion_port shall be freed. ]
            do
            {
                int32_t current_val = interlocked_compare_exchange(&completion_port->recv_data_access, 1, 0);
                if (current_val == 0)
    ...
    ON_COMPLETION_PORT_EVENT_COMPLETE event_callback, void* event_callback_ctx)
{
    int result;
    if (
        // Codes_COMPLETION_PORT_LINUX_11_016: [ If completion_port is NULL, completion_port_add shall return a non-NULL value. ]
        completion_port == NULL || 
        // Codes_COMPLETION_PORT_LINUX_11_017: [ If socket is INVALID_SOCKET, completion_port_add shall return a non-NULL value. ]
        socket == INVALID_SOCKET ||
        // Codes_COMPLETION_PORT_LINUX_11_018: [ If event_callback is NULL, completion_port_add shall return a non-NULL value. ]
        event_callback == NULL)
    {
    ...
    {
        // Codes_COMPLETION_PORT_LINUX_11_019: [ completion_port_add shall ensure the thread completion flag is not set. ]
        if (interlocked_add(&completion_port->worker_thread_continue, 0) != 0)
        {
            LogError("Invalid call sequence completion port module is closing");
            result = MU_FAILURE;
        }
        else
        {
            // Codes_COMPLETION_PORT_LINUX_11_020: [ completion_port_add shall increment the ongoing call count value to prevent close. ]
            (void)interlocked_increment(&completion_port->pending_calls);

            // Codes_COMPLETION_PORT_LINUX_11_021: [ completion_port_add shall allocate a EPOLL_THREAD_DATA object to store thread data. ]
            ...
            else
            {
                ...
                #endif
                // Codes_COMPLETION_PORT_LINUX_11_022: [ completion_port_add shall add the EPOLL_THREAD_DATA object to a list for later removal. ]
                ...
                {
                    // Codes_COMPLETION_PORT_LINUX_11_023: [ completion_port_add shall add the socket in the epoll system by calling epoll_ctl with EPOLL_CTL_MOD along with the epoll_op variable. ]
                    if (epoll_ctl(completion_port->epoll, EPOLL_CTL_MOD, epoll_thread_data->socket, &ev) < 0)
                    {
                        if (errno == ENOENT)
                        {
                            // Codes_COMPLETION_PORT_LINUX_11_024: [ If the epoll_ctl call fails with ENOENT, completion_port_add shall call epoll_ctl again with EPOLL_CTL_ADD. ]
                    ...
                    {
                        // Codes_COMPLETION_PORT_LINUX_11_026: [ On success, completion_port_add shall return 0. ]
                        result = 0;
                        // Codes_COMPLETION_PORT_LINUX_11_025: [ completion_port_add shall decrement the ongoing call count value to unblock close. ]
                        (void)interlocked_decrement(&completion_port->pending_calls);
                        wake_by_address_single(&completion_port->pending_calls);
                        goto all_ok;
                    }
            ...
            // Codes_COMPLETION_PORT_LINUX_11_027: [ If any error occurs, completion_port_add shall fail and return a non-zero value. ]
            result = MU_FAILURE;
            (void)interlocked_decrement(&completion_port->pending_calls);
            wake_by_address_single(&completion_port->pending_calls);
        }
...
{
    if (
        // Codes_COMPLETION_PORT_LINUX_11_028: [ If completion_port is NULL, completion_port_remove shall return. ]
        completion_port == NULL || 
        // Codes_COMPLETION_PORT_LINUX_11_029: [ If socket is INVALID_SOCKET, completion_port_remove shall return. ]
        socket == INVALID_SOCKET)
    {
    ...
    {
        // Codes_COMPLETION_PORT_LINUX_11_030: [ completion_port_remove shall remove the underlying socket from the epoll by calling epoll_ctl with EPOLL_CTL_DEL. ]
