...
#define PTR_STRUCT_TYPE_TYPEDEF(T) typedef struct PTR_STRUCT_TAG_TYPE_NAME(T) {T pointer; THANDLE_PTR_FREE_FUNC_TYPE_NAME(T) the_free;} PTR_STRUCT_TYPE_NAME(T);
...
#define THANDLE_PTR_DEFINE(T) \
    ...
    static void inner_dispose(PTR_STRUCT_TYPE_NAME(T)* ptr)                                                         \
    {                                                                                                               \
        ptr->the_free(ptr->pointer);                                                                                \
    }                                                                                                               \
    THANDLE(PTR_STRUCT_TYPE_NAME(T)) create(T source, void(*dispose)(T) )                                           \
    {                                                                                                               \
        (void)dispose;                                                                                              \
        PTR_STRUCT_TYPE_NAME(T) temp = { .pointer = source, .the_free=dispose };                                    \
        return THANDLE_CREATE_FROM_CONTENT(PTR_STRUCT_TYPE_NAME(T))(&temp, inner_dispose, NULL);                    \
    }
