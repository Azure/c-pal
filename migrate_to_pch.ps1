...
function Update-CmakeFile {
    ...
    if ($content -match $pattern) {
        $fullMatch = $matches[0]
        $buildTestArtifactsCall = $fullMatch -replace '\)$', ''  # Remove closing parenthesis
        ...
        # Determine if this is a single-line call (no newlines within the call)
        $isSingleLine = $cleanedCall -notmatch "`r?`n"
        
        if ($isSingleLine) {
            # For single-line calls, convert to multi-line format
            $newCall = $cleanedCall + " `r`n    ENABLE_TEST_FILES_PRECOMPILED_HEADERS `"`${CMAKE_CURRENT_LIST_DIR}/$PchFileName`"`r`n)"
        } else {
            # For multi-line calls, add the parameter before the closing parenthesis
            # Find the appropriate indentation by looking at existing parameters
            $lines = $cleanedCall -split "`r?`n"
            $indent = "    "  # Default indentation
            
            # Try to find indentation from existing parameters
            for ($i = 1; $i -lt $lines.Count; $i++) {
                if ($lines[$i] -match '^(\s+)' -and $lines[$i].Trim() -ne '') {
                    $indent = $matches[1]
                    break
                }
            }
            
            $newCall = $cleanedCall + "`r`n$indent" + "ENABLE_TEST_FILES_PRECOMPILED_HEADERS `"`${CMAKE_CURRENT_LIST_DIR}/$PchFileName`"`r`n)"
        }
