...
MOCKABLE_FUNCTION(, int, threadpool_schedule_work_item, THANDLE(THREADPOOL), threadpool, THANDLE(THREADPOOL_WORK_ITEM_CONTEXT), threadpool_work_item);
...
**SRS_THREADPOOL_WIN32_05_006: [** Otherwise `threadpool_create_work_item` shall allocate a context of type `THANDLE(THREADPOOL_WORK_ITEM_CONTEXT)` with the dispose function being `threadpool_dispose_work_item`. **]**

**SRS_THREADPOOL_WIN32_05_007: [** If any error occurs, `threadpool_create_work_item` shall fail and return a `NULL` value. **]**

**SRS_THREADPOOL_WIN32_05_008: [** `threadpool_create_work_item` shall create `threadpool_work_item_context` member variable `ptp_work` of type `PTP_WORK` by calling `CreateThreadpoolWork` to set the callback function as `on_work_callback_v2`. **]**

**SRS_THREADPOOL_WIN32_05_009: [** If there are no errors then this `threadpool_work_item_context` of type `THANDLE(THREADPOOL_WORK_ITEM_CONTEXT)` would be returned indicating a succcess to the caller. **]**
...
**SRS_THREADPOOL_WIN32_05_012: [** If `threadpool_work_item_context` is NULL, `threadpool_schedule_work_item` shall fail and return a non-zero value. **]**
...
```c
static void threadpool_dispose_work_item(THANDLE(THREADPOOL_WORK_ITEM_CONTEXT) threadpool_work_item_context);
```

`threadpool_dispose_work_item` closes the `ptp_work` member variable in `threadpool_work_item_context`.
