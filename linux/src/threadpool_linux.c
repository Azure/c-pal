    ...
    else
    {
        ...
        else
        {
            do
            {
                ...
                {
                    THREADPOOL_TASK* task_item = &threadpool_ptr->task_array[insert_pos];
                    /* Codes_SRS_THREADPOOL_LINUX_05_019: [ threadpool_schedule_work_item shall increment the pending_work_item_count and copy its address to pending_work_item_count_ptr into insert position in the task array. ]*/
                    ...
                    /* Codes_SRS_THREADPOOL_LINUX_05_020: [ threadpool_schedule_work_item shall copy the work_function and work_function_context from threadpool_work_item into insert position in the task array. ]*/
                    ...
                    /* Codes_SRS_THREADPOOL_LINUX_05_021: [ threadpool_schedule_work_item shall set the task_state to TASK_WAITING and then release the shared SRW lock. ]*/
                    (void)interlocked_exchange(&task_item->task_state, TASK_WAITING);
                    srw_lock_release_shared(threadpool_ptr->srw_lock);

                    /* Codes_SRS_THREADPOOL_LINUX_05_022: [ threadpool_schedule_work_item shall unblock the threadpool semaphore by calling sem_post. ]*/
                    sem_post(&threadpool_ptr->semaphore);

                    /* Codes_SRS_THREADPOOL_LINUX_05_023: [ threadpool_schedule_work_item shall set the return variable to 0 to indicate success. ]*/
                    result = 0;
                    break;
                }
            } while (true);
            /* Codes_SRS_THREADPOOL_LINUX_05_024: [ threadpool_schedule_work shall call sm_exec_end. ]*/
            sm_exec_end(threadpool_ptr->sm);
        }
    }
    /* Codes_SRS_THREADPOOL_LINUX_05_025: [ threadpool_schedule_work_item shall return with the contents of the value of the return variable. ]*/
    return result;
}
...
{
    if (
        /* Codes_SRS_THREADPOOL_LINUX_05_026: [ If threadpool is NULL, threadpool_destroy_work_item shall fail. ]*/
        (threadpool == NULL) ||
        /* Codes_SRS_THREADPOOL_LINUX_05_027: [ If threadpool_work_item is NULL, threadpool_destroy_work_item shall fail. ]*/
        (threadpool_work_item == NULL)
        )
    {
        LogError("Invalid arguments: THANDLE(THREADPOOL) threadpool: %p, THREADPOOL_WORK_ITEM_HANDLE threadpool_work_item: %p", threadpool, threadpool_work_item);
    ...
    else
    {
        ...
        /* Codes_SRS_THREADPOOL_LINUX_05_028: [ threadpool_destroy_work_item shall call sm_exec_begin ]*/
        SM_RESULT sm_result = sm_exec_begin(threadpool_ptr->sm);
        if (sm_result != SM_EXEC_GRANTED)
        {
            /* Codes_SRS_THREADPOOL_LINUX_05_029: [ If sm_exec_begin returns SM_EXEC_REFUSED, threadpool_destroy_work_item shall fail. ]*/
            LogError("sm_exec_begin failed : %" PRI_MU_ENUM, MU_ENUM_VALUE(SM_RESULT, sm_result));
        }
        else
        {
            /* Codes_SRS_THREADPOOL_LINUX_05_030: [ threadpool_destroy_work_item shall wait for pending_work_item_count to become 0. ]*/
            if (InterlockedHL_WaitForValue(&threadpool_work_item->pending_work_item_count, 0, UINT32_MAX) == INTERLOCKED_HL_OK)
            {
                /* Codes_SRS_THREADPOOL_LINUX_05_031: [ threadpool_destroy_work_item shall acquire the SRW lock in shared mode by calling srw_lock_acquire_shared. ]*/
                srw_lock_acquire_shared(threadpool_ptr->srw_lock);
                /* Codes_SRS_THREADPOOL_LINUX_05_032: [ threadpool_destroy_work_item shall free the memory allocated to the work item of type THREADPOOL_WORK_ITEM_HANDLE created in threadpool_create_work_item. ]*/
                free(threadpool_work_item);
                /* Codes_SRS_THREADPOOL_LINUX_05_033: [ threadpool_destroy_work_item shall release the shared SRW lock. ]*/
                srw_lock_release_shared(threadpool_ptr->srw_lock);
            }
            else
            {
                /* Codes_SRS_THREADPOOL_LINUX_05_034: [ If InterlockedHL_WaitForValue returns error then log the error and terminate. ]*/
        ...
        /* Codes_SRS_THREADPOOL_LINUX_05_035: [ threadpool_destroy_work_item shall call sm_exec_end. ]*/
        sm_exec_end(threadpool_ptr->sm);
    }
}