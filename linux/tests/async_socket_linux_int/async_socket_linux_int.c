...
TEST_FUNCTION(send_and_receive_1_byte_succeeds)
{
    ...
    int port_num = TEST_PORT;
    setup_server_socket(port_num, &listen_socket);
    setup_client_sockets(port_num, &client_socket, &listen_socket, &accept_socket);
    ...
    //dump_bytes("1 byte receive: ", receive_payload_buffers[0].buffer, receive_payload_buffers[0].length);
    //dump_bytes("   1 byte send: ", send_payload_buffers[0].buffer, send_payload_buffers[0].length);
...
// TODO: Add test to send data to multiple clients
